...
for i in range(1,41):
    ...
    if i<=20:
        if i <=15:
            data_train_label.append('One')
        data_ones.append(np.fft.fft(sound)[:17])
    else:
        if i <=35:
            data_train_label.append('Two')
        data_twos.append(np.fft.fft(sound)[:17])
...
pipe.fit(data_train,data_train_label.real.ravel())
