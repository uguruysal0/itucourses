...
--onelines getWords piece of art for me!
getWords'::Trie->[String]
getWords' (Trie e c)
    ...
    case head c of
        ...
        'f' ->  do  putStrLn "Enter word/prefix"
                    ...
                    case prefix word t of 
                        Nothing -> (putStrLn $ "With prefix '" ++ word ++ "' no word found") >> programloop t
                        Just a  -> (putStrLn $ unlines a)  >>  programloop t
        'p' ->  do  (putStrLn $ unlines $ getWords' t ) >> programloop t
        'e' -> return ()
        _   -> programloop t
